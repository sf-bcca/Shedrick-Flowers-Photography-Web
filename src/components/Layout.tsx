...
export const Header: React.FC<{ transparent?: boolean }> = ({
  ...
  return (
    <>
      ...
      />
      <header
        className={`fixed top-0 left-0 right-0 z-50 border-b border-white/10 transition-colors duration-300 ${transparent ? "bg-transparent backdrop-blur-sm" : "bg-background-dark/90 backdrop-blur-md"}`}
      >
        <div className="px-4 md:px-10 py-4 flex items-center justify-between w-full max-w-[1440px] mx-auto">
          ...
          <div className="hidden md:flex flex-1 justify-end gap-8 items-center text-slate-300">
            ...
            <Link
              to="/contact"
              className="flex items-center justify-center rounded-lg h-10 px-5 bg-primary hover:bg-blue-600 transition-colors text-white text-sm font-bold tracking-wide shadow-lg shadow-primary/20"
            >
              Book a Session
            </Link>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-slate-400 text-sm max-w-xs">
              Capturing the soul of the moment with professional photography
              services worldwide.
            </p>
          </div>

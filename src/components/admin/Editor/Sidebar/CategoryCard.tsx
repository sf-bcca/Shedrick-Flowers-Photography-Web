             ...
             <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {PRESET_CATEGORIES.map(cat => (
                    <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors group-focus-within:ring-2 group-focus-within:ring-primary group-focus-within:ring-offset-2 group-focus-within:ring-offset-[#1a2232] ${category === cat ? 'bg-primary border-primary' : 'border-slate-500 group-hover:border-white'}`}>

...
2.  **Install Dependencies**: Use `pnpm` to install dependencies (strict peer dependency handling is managed automatically):
    ```bash
    pnpm install
    ```
...
5.  **Test**: Ensure the application runs without errors (`pnpm dev`) and builds successfully (`pnpm run build`).

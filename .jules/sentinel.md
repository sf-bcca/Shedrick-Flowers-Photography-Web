...
## 2024-05-27 - [Input Validation for Edge Function]
**Vulnerability:** The `gemini-chat` Edge Function accepted any JSON payload in `messages` without validation, exposing the app to DoS, token exhaustion, and potential cost injection attacks.
**Learning:** Serverless functions are publicly accessible endpoints; never trust the client to validate input. Always enforce strict schema and limits on the server side.
**Prevention:** Implemented strict input validation: verified `messages` is an array (max 20 items), enforced role values ('user'/'model'), and limited message length (max 2000 chars) before processing.

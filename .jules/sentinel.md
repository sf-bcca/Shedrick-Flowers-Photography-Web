...
## 2024-05-24 - [Unvalidated Edge Function Input]
**Vulnerability:** The `gemini-chat` Edge Function accepted raw JSON input without validation, allowing potential DoS (large payloads) or cost injection (many messages).
**Learning:** Serverless functions are often exposed directly to the internet; assuming trusted input from the client is a common pitfall.
**Prevention:** Implemented strict input validation (array check, length limits, type checks) at the entry point of the function.

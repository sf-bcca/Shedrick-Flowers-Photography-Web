...
_Note: Supabase Edge Functions (in `supabase/functions/`) use Deno and manage their own dependencies via `import_map.json` or direct URL imports. You do not need to run any package install command (like `pnpm install`) inside the functions directory._
...
_Note: Since this is a Client-Side Rendered (CSR) app (Vite + React), the Docker container effectively serves static files generated by `pnpm build`. You do not need the Node.js runtime in the final container, just a web server like Nginx._

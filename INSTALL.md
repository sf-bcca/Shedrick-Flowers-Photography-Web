...
- **pnpm**: Enabled via Corepack (`corepack enable`) or installed globally (`npm i -g pnpm`).
...
| `VITE_WEB3FORMS_ACCESS_KEY` | ❌ Optional | Access key for [Web3Forms](https://web3forms.com/) contact form email delivery. **Highly Recommended.** If omitted, contact submissions are securely saved to the `contact_submissions` table in Supabase, but you will not receive email notifications. |
